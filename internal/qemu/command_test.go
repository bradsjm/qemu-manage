...
func renderFixture() (*model.Config, backend.RuntimePaths) {
	return &model.Config{
			...
			Disks: []model.DiskConfig{
				{Path: "disks/root,one.qcow2", Format: "qcow2", Cache: "none", Serial: "root,serial", BootIndex: 1},
...
func TestRenderUserNetworkGolden(t *testing.T) {
	...
	want := backend.Command{Path: "/vms/ha/bin/qemu-system-aarch64", Args: []string{
		...
		"-drive", "if=none,media=disk,id=disk0,file.filename=/vms/ha/disks/root,,one.qcow2,format=qcow2,aio=threads,cache=none,discard=unmap,detect-zeroes=unmap",
		"-device", "virtio-blk-pci,drive=disk0,serial=root,,serial,bootindex=1",
		"-drive", "if=none,media=disk,id=disk1,file.filename=/images/recovery.raw,format=raw,aio=threads,readonly=on",
