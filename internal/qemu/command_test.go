...
func renderFixture() (*model.Config, backend.RuntimePaths) {
	return &model.Config{
			...
			Disks: []model.DiskConfig{
				{Path: "disks/root,one.qcow2", Format: "qcow2", Serial: "root,serial", BootIndex: 1},
			...
			VMDir: "/vms/ha", QMP: "/run/qmp,0.sock", QGA: "/run/qga.sock", Console: "/run/console.sock", SerialLog: "/logs/serial,0.log",
		}
}
...
func TestRenderUserNetworkGolden(t *testing.T) {
	...
	want := backend.Command{Path: "/vms/ha/bin/qemu-system-aarch64", Args: []string{
		...
		"-qmp", "unix:/run/qmp,,0.sock,server=on,wait=off", "-device", "virtio-serial-pci", "-chardev", "socket,id=qga0,path=/run/qga.sock,server=on,wait=off",
...
func TestRenderSocketVMNetGolden(t *testing.T) {
	...
	want := backend.Command{Path: "/opt/socket_vmnet/bin/socket_vmnet_client", Args: []string{
		...
		"-chardev", "socket,id=console0,path=/run/console.sock,server=on,wait=off,logfile=/logs/serial,,0.log,logappend=on", "-serial", "chardev:console0", "-qmp", "unix:/run/qmp,,0.sock,server=on,wait=off",
